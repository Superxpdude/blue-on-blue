...
class Jail(commands.GroupCog, group_name="jail"):
	...
	async def jail(
		...
		modChannel = await self.bot.serverConfig.channel_mod_activity.get(
			interaction.guild
		)
		assert modChannel is not None
		jailRole = await self.bot.serverConfig.role_timeout.get(interaction.guild)
		...
		async with self.bot.db.connect() as db:
			async with db.connection.cursor() as cursor:
				...
				releaseTimeStamp = round(
					(discord.utils.utcnow() + timeDelta).timestamp()
				)
				...
				if view.response:
					...
					await cursor.execute(
						"SELECT * FROM jail WHERE server_id = :serverID AND user_id = :userID",
					...
					if userData is None:
						...
						for (
							role
						) in user.roles:  # Make a list of roles that the bot can remove
							if (
						...
						try:
							...
							await cursor.execute(
								"INSERT OR REPLACE INTO jail (server_id, user_id, release_time) VALUES \
							(:serverID, :userID, :releaseTime)",
								{
							...
							await interaction.followup.send(
								f"User {user.mention} has been jailed.", ephemeral=True
							)
							...
							await interaction.followup.send(
								"Failed to assign roles to jail user."
							)
					else:
						# User in jail DB. Only update release time.
						await cursor.execute(
							"UPDATE jail SET release_time = :releaseTime WHERE server_id = :serverID AND user_id = :userID",
							{
								"serverID": interaction.guild.id,
								"userID": user.id,
								"releaseTime": releaseTimeStamp,
						...
						await interaction.followup.send(
							f"Updated release time for user {user.mention}.",
							ephemeral=True,
						)
						await modChannel.send(
							f"Jail for user {user.mention} has been modified by {interaction.user.mention} to {timeText} {time_unit}.",
							allowed_mentions=discord.AllowedMentions.none(),
						)
					...
					await interaction.followup.send(
						"Pending jail action has timed out", ephemeral=True
					)
	...
	async def release(self, interaction: discord.Interaction, user: discord.Member):
		...
		async with self.bot.db.connect() as db:
			async with db.connection.cursor() as cursor:
				...
				await cursor.execute(
					"SELECT user_id, release_time FROM jail WHERE server_id = :serverID AND user_id = :userID",
					{"serverID": interaction.guild.id, "userID": user.id},
				)
				userData = await cursor.fetchone()
				if userData is None:
					# User not jailed
					await interaction.response.send_message(
						f"I could not find {user.mention} in the jail list.",
						ephemeral=True,
						allowed_mentions=discord.AllowedMentions.none(),
					)
					return
				...
				view = blueonblue.views.ConfirmViewDanger(
					interaction.user, confirm="Release"
				)
				jailEmbed = discord.Embed(
				...
				await interaction.response.send_message(
					f"{interaction.user.mention}, you are about to release the following user.",
					view=view,
					embed=jailEmbed,
				)
				view.message = await interaction.original_response()
				...
				if view.response:
					...
					await cursor.execute(
						"SELECT role_id FROM user_roles WHERE server_id = :serverID AND user_id = :userID",
						{"serverID": interaction.guild.id, "userID": user.id},
					)
					...
					await cursor.execute(
						"DELETE FROM jail WHERE server_id = :serverID AND user_id = :userID",
						{"serverID": interaction.guild.id, "userID": user.id},
					)
					...
					try:
						...
						await interaction.followup.send(
							f"User {user.mention} has been released from jail.",
							ephemeral=True,
						)
						await modChannel.send(
							f"User {user.mention} has been released from jail by {interaction.user.mention}",
							allowed_mentions=discord.AllowedMentions.none(),
						)
					except:
						await modChannel.send(
							f"Error assigning roles when releasing user {user.mention} from jail. Please assign roles manually.",
							allowed_mentions=discord.AllowedMentions.none(),
						)
						_log.warning(
							f"Failed to assign roles to release user from jail. User: [{user.id}] Roles: [{userRoles}]"
						)
					...
					# Notify the user that the action timed out
					await interaction.followup.send(
						"Pending release action has timed out", ephemeral=True
					)
	...
	async def list(self, interaction: discord.Interaction):
		...
		async with self.bot.db.connect() as db:
			async with db.connection.cursor() as cursor:
				...
				await cursor.execute(
					"SELECT user_id, release_time FROM jail WHERE server_id = :serverID",
					{"serverID": interaction.guild.id},
				)
				...
				jailEmbed = discord.Embed(
					title="Jailed Users", colour=JAIL_EMBED_COLOUR
				)
				...
				for userData in usersData:
					...
					jailEmbed.add_field(
						name=userName,
						value=f"<t:{userData['release_time']}:F>",
						inline=False,
					)
	...
	async def jail_loop(self):
		...
		async with self.bot.db.connect() as db:
			async with db.connection.cursor() as cursor:
				...
				# Read the DB
				await cursor.execute(
					"SELECT server_id, user_id FROM jail WHERE release_time < :time",
					{"time": timeStamp},
				)
				...
				for userData in releaseData:
					_log.debug(
						f"Releasing user {userData['user_id']} from server {userData['server_id']}."
					)
					...
					if guild is not None:
						...
						modChannel = (
							await self.bot.serverConfig.channel_mod_activity.get(guild)
						)
						# Get the jail role
						jailRole = await self.bot.serverConfig.role_timeout.get(guild)
						...
						if user is not None:
							...
							try:
								await user.add_roles(
									*userRoles, reason="Jail timeout expired"
								)
								assert jailRole is not None
								await user.remove_roles(
									jailRole, reason="Jail timeout expired"
								)
								if modChannel is not None:
									await modChannel.send(
										f"User {user.mention} has been released from jail due to timeout expiry.",
										allowed_mentions=discord.AllowedMentions.none(),
									)
							except:
								if modChannel is not None:
									await modChannel.send(
										f"Error assigning roles when releasing user {user.display_name} from jail.",
										allowed_mentions=discord.AllowedMentions.none(),
									)
								_log.warning(
									f"Failed to assign roles to release user from jail. Guild: [{guild.id}]. User: [{user.id}]. Roles: {userRoles}"
								)
						else:
							...
							if modChannel is not None:
								...
								await modChannel.send(
									f"Failed to release user `{userInfo['display_name']}` from jail, user may no longer be present in the server",
									allowed_mentions=discord.AllowedMentions.none(),
								)

				# Delete data of released users
				await cursor.execute(
					"DELETE FROM jail WHERE release_time < :time", {"time": timeStamp}
				)
				# Commit our changes
				await db.commit()
