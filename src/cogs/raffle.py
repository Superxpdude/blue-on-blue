...
def weighted_sample_without_replacement(
	population: list | tuple, weights: list | tuple, k: int, rng=random
) -> list:
...
class RaffleObject:
		...
		if len(eligible) > 0:
			...
			if self.mission:
				async with self.view.bot.db.connect() as db:
					_log.debug(
						f"Beginning weighted raffle. Guild: {self.view.guild.id}"
					)
					weights = [
						await db.raffleWeight.getWeight(self.view.guild.id, p.id)
						for p in self.participants
					]
					_log.debug(
						f"Raffle participants: {[f'({e.display_name}|{e.id})' for e in eligible]}"
					)
					_log.debug(f"Raffle weights: {weights}")
					winners: tuple[discord.Member, ...] = tuple(
						weighted_sample_without_replacement(
							eligible, weights, winnerCount
						)
					)
					_log.debug(
						f"Raffle winners: {[f'({w.display_name}|{w.id})' for w in winners]}"
					)
					return winners
			else:
	...
	async def endRaffleEmbed(
		self, winners: tuple[discord.User | discord.Member, ...] | None = None
	) -> discord.Embed:
		...
		embed = discord.Embed(
			title=f"🎉 Mission Raffle: {self.name}"
			if self.mission
			else f"🎉 Raffle: {self.name}",
		...
		if self.participantCount() < 1:
			...
			if len(winners) < 1:
				...
				embed.add_field(
					name="Winners",
					value=", ".join(map(lambda x: x.mention, winners)),
					inline=False,
				)
			...
			if self.mission:
				...
				async with self.view.bot.db.connect() as db:
					for u in self.participants:
						participantStrings.append(
							f"{u.mention}`({await db.raffleWeight.getWeight(self.view.guild.id, u.id):.1f})`"
						)
				embed.add_field(
					name="Participants",
					value=", ".join(participantStrings),
					inline=False,
				)
			else:
				embed.add_field(
					name="Participants",
					value=", ".join(map(lambda x: x.mention, self.participants)),
					inline=False,
				)
...
class RaffleView(discord.ui.View):
	...
	def build_embed(self) -> discord.Embed:
		...
		for r in self.raffles:
			if r.winners > 1:
				embed.add_field(
					name=f"{r.name} ({r.winners} winners)",
					value=r.participantCount(),
					inline=True,
				)
			else:
				embed.add_field(
					name=f"{r.name}", value=r.participantCount(), inline=True
				)
	...
	async def update_embed(self) -> None:
		...
		if not self.updating:
			...
			if (self.lastUpdateTime is not None) and (
				(discord.utils.utcnow() - self.lastUpdateTime).total_seconds() < 10
			):
...
class Raffle(commands.Cog, name="Raffle"):
	...
	raffleGroup = app_commands.Group(
		name="raffle", description="Raffle commands", guild_only=True
	)
		...
		*,
		...
		if weighted:
			async with self.bot.db.connect() as db:
				for w in allWinners:
					await db.raffleWeight.setWeight(
						interaction.guild.id,
						w.id,
						1.0
						- (
							await self.bot.serverConfig.raffleweight_increase.get(
								interaction.guild.id
							)
						),
					)
	...
	async def multiRaffle(
		...
		*,
		...
		if len(raffleList) > 10:
			await interaction.response.send_message(
				"You cannot run more than 10 raffles at once", ephemeral=True
			)
			return
	...
	@app_commands.guild_only()  # No point in running raffles in DMs
	async def mission_raffle_single(
		self,
		interaction: discord.Interaction,
		raffle_name: str,
		duration: app_commands.Range[int, 15, 600],
		...
		winners : app_commands.Range[int, 0, None] | None, optional
			How many winners should be selected
		"""
		return await self.singleRaffle(
			interaction, raffle_name, duration, winners, weighted=True
		)

	@missionRaffleGroup.command(name="multi")
	@app_commands.guild_only()  # No point in running raffles in DMs
	async def mission_raffle_multi(
		self,
		interaction: discord.Interaction,
		duration: app_commands.Range[int, 15, 600],
		raffles: str,
		...
		raffles : str
			Raffle info. Formatted as *RaffleName*:*WinnerCount(Optional),*RaffleName*,... Maximum of 10 raffles at once.
		"""
		return await self.multiRaffle(
			interaction, duration, raffles, exclusive, weighted=True
		)
	...
	async def weight_get(self, interaction: discord.Interaction, user: discord.Member):
	...
	async def weight_set(
		self, interaction: discord.Interaction, user: discord.Member, weight: float
	):
		...
		async with self.bot.db.connect() as db:
			...
			embed.set_author(name=user.display_name, icon_url=user.display_avatar)
			_log.info(
				f"[{interaction.guild.name}|{interaction.guild.id}] Raffle weight for ({user.display_name}|{user.id})"
				f" set to {weight} by ({interaction.user.display_name}|{interaction.user.id})"
			)
			await interaction.response.send_message(embed=embed)

	@raffleWeightGroup.command(name="increase")
	async def weight_increase(
		self, interaction: discord.Interaction, user: discord.Member, increase: float
	):
		...
		"""
		assert interaction.guild is not None
		maxWeight = await self.bot.serverConfig.raffleweight_max.get(
			interaction.guild.id
		)

		async with self.bot.db.connect() as db:
			await db.raffleWeight.increaseWeight(
				interaction.guild.id, user.id, increase, maxWeight
			)
			weight = await db.raffleWeight.getWeight(interaction.guild.id, user.id)
			await db.commit()
			embed = discord.Embed(
