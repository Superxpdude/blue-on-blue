...
class Pings(commands.Cog, name="ping"):
	...
	async def ping_autocomplete(self, interaction: discord.Interaction, current: str):
		...
		if (interaction.guild is None) or (interaction.guild.id not in self.pingCache):
			...
			return [
				...
				for ping in self.pingCache[interaction.guild.id]
				if current.lower() in ping.lower()
			][:25]
	...
	async def create_ping_embed(
		...
		*,
		...
		if title_prefix is not None:
			# Prefix present
			embedTitle = (
				f"{title_prefix} Ping: {pingName} | Users: {len(pingUserNames)}"
			)
		else:
		...
		if len(pingAliases) > 0:
			embed.add_field(
				name="Aliases", value=f"```{', '.join(pingAliases)}```", inline=True
			)
	...
	pingGroup = app_commands.Group(
		name="ping_manage", description="Ping management commands", guild_only=True
	)
	...
	async def ping(self, interaction: discord.Interaction, tag: str):
		...
		if san_check is not None:  # Validate our tag first
			await interaction.response.send_message(
				f"{interaction.user.mention}: {san_check}"
			)
			return
		...
		async with self.bot.db.connect() as db:
			response = None
			ping_id = await db.pings.get_id(
				tag, interaction.guild.id
			)  # Get the ID of the ping (or none if it doesn't exist)
			if ping_id is None:
				# Ping does not exist
				...
				if len(pingMentions) > 0:
					...
					response = (
						f"{interaction.user.mention} has pinged `{tag}`: "
						+ " ".join(pingMentions)
	...
	async def pingme(self, interaction: discord.Interaction, tag: str):
		...
		async with self.bot.db.connect() as db:
			...
			if await db.pings.exists(tag, interaction.guild.id):  # Ping exists
				...
				if await db.pings.has_user(
					tag, interaction.guild.id, interaction.user.id
				):
					# User already in ping
					success = await db.pings.remove_user(
						tag, interaction.guild.id, interaction.user.id
					)
					if success:
					...
					success = await db.pings.add_user(
						tag, interaction.guild.id, interaction.user.id
					)
					if success:
	...
	@app_commands.describe(
		mode="Operation mode. 'All' lists all pings. 'Me' returns your pings."
	)
	async def pinglist(
		self, interaction: discord.Interaction, mode: Literal["all", "me"] = "all"
	):
			...
			async with db.connection.cursor() as cursor:
				...
				if mode == "me":  # Grab a list of pings that the user is in
					...
					await cursor.execute(
						"SELECT ping_id FROM ping_users WHERE (server_id = :server_id AND user_id = :user_id)",
						{
					...
					)  # Get the list of pings from the database
					pingIDs: list[int] = []
					...
					if len(pingIDs) > 0:
						...
						await cursor.execute(
							f"SELECT ping_name FROM pings WHERE id IN ({','.join('?' * len(pingIDs))})",
							tuple(pingIDs),
						)
					...
					await cursor.execute(
						"SELECT ping_name FROM pings WHERE server_id = :server_id AND alias_for IS NULL",
						{"server_id": interaction.guild.id},
					)
		...
		async with self.bot.db.connect() as db:
			...
			if await db.pings.exists(tag, interaction.guild.id):
				...
				if len(pingUserNames) > 0:
					...
					pingEmbed = await self.create_ping_embed(
						db, pingID, interaction.guild
					)
				else:
				...
				pingResults = await db.pings.server_pings(
					interaction.guild.id, search=tag
				)
				# Now that we have our ping names, we can form our response
				if len(pingResults) > 0:
					# We have at least one ping response
	...
	@app_commands.autocomplete(tag=ping_autocomplete)
	async def pingalias(
		self, interaction: discord.Interaction, alias: str, tag: str | None = None
	):
			...
			if tag is not None:
				...
				if await db.pings.exists(alias, interaction.guild.id):
					...
					if await db.pings.is_alias(alias, interaction.guild.id):
						...
						if primaryID is not None:
							...
							await interaction.response.send_message(
								f"The alias `{alias}` already exists for the ping `{primaryName}`. Please clear the alias before trying to reassign it.",
								ephemeral=True,
							)
						else:
							await interaction.response.send_message(
								f"The alias `{alias}` already exists. Please clear the alias before trying to reassign it.",
								ephemeral=True,
							)
					else:
						# Ping is not an alias
						await interaction.response.send_message(
							f"A ping already exists with the tag `{alias}`. If you want to turn this tag into an alias, the ping must be deleted first.",
							ephemeral=True,
						)
				else:
					...
					if primaryID is not None:
						primaryName = await db.pings.get_name(primaryID)
						await db.pings.create_alias(
							alias, primaryID, interaction.guild.id
						)
						await db.commit()  # Commit changes
						await interaction.response.send_message(
							f"Alias `{alias}` created for ping `{primaryName}`"
						)
					else:
						await interaction.response.send_message(
							f"The ping `{tag}` does not exist. Alias could not be created.",
							ephemeral=True,
						)
				...
				if aliasExists:
					...
					if primaryID is not None:
						...
						msg = (
							f"Alias `{alias}` has been removed for ping `{primaryName}`"
						)
						...
						await interaction.response.send_message(
							f"Alias `{alias}` has been removed"
						)

				else:
					await interaction.response.send_message(
						f"The alias `{alias}` does not exist. If you are trying to create an alias, please specify a ping to bind the alias to.",
						ephemeral=True,
					)
	...
	@app_commands.autocomplete(merge_from=ping_autocomplete, merge_to=ping_autocomplete)
	async def pingmerge(
		self, interaction: discord.Interaction, merge_from: str, merge_to: str
	):
		"""Merges two pings"""
		assert interaction.guild is not None
		...
		async with self.bot.db.connect() as db:
			...
			if fromName is None or fromID is None:
				# "From" ping not found
				await interaction.response.send_message(
					f"The ping `{merge_from}` does not exist. Please specify a valid ping to be merged.",
					ephemeral=True,
				)
				return
			if toName is None or toID is None:
				# "To" ping not found
				await interaction.response.send_message(
					f"The ping `{merge_to}` does not exist. Please specify a valid ping to merge to.",
					ephemeral=True,
				)
				return
			...
			pingEmbeds = [
				await self.create_ping_embed(
					db, fromID, interaction.guild, title_prefix="Merge from"
				),
				await self.create_ping_embed(
					db, toID, interaction.guild, title_prefix="Merge to"
				),
			]
			await interaction.response.send_message(
				messageText, view=view, embeds=pingEmbeds
			)
			...
			if view.response:
				...
				await interaction.followup.send(
					f"Ping `{fromName}` has been successfully merged into {toTextNew}."
				)

			elif not view.response:
				# Action cancelled
				await interaction.followup.send("Merge action cancelled.")

			else:
				# Notify the user that the action timed out
				await interaction.followup.send(
					"Pending ping merge has timed out", ephemeral=True
				)
	...
	async def pingdelete(self, interaction: discord.Interaction, tag: str):
		...
		async with self.bot.db.connect() as db:
			...
			if pingID is None:
				await interaction.response.send_message(
					f"I could not find a ping for the tag: `{tag}`", ephemeral=True
				)
				return
			...
			view = blueonblue.views.ConfirmViewDanger(
				interaction.user, confirm="Delete"
			)
			...
			if view.response:
				...
				await self._update_ping_cache(
					db, interaction.guild.id
				)  # Update the cache
				await interaction.followup.send(
					f"The ping `{pingName}` has been permanently deleted."
				)
			elif not view.response:
				# Action cancelled
				await interaction.followup.send("Delete action cancelled.")
			else:
				# Notify the user that the action timed out
				await interaction.followup.send(
					"Pending ping delete has timed out", ephemeral=True
				)
	...
	async def pingpurge(
		...
		async with self.bot.db.connect() as db:
			...
			pingNames = []
			for p in await db.pings.server_pings(
				interaction.guild.id, beforeTime=timeThreshold
			):
				# Ping names cannot be null/None
				if (
					await db.pings.count_active_users(p, interaction.guild)
					< user_threshold
				):
					pingNames.append(p)
			...
			if len(pingNames) > 0:
				...
				pingEmbed = discord.Embed(
					title="Pings pending deletion",
					colour=PING_EMBED_COLOUR,
					description=f"```{', '.join(sorted(pingNames, key=str.casefold))}```",
				)
				view = blueonblue.views.ConfirmViewDanger(
					interaction.user, confirm="Purge"
				)
				await interaction.response.send_message(msg, embed=pingEmbed, view=view)
				view.message = await interaction.original_response()
				await view.wait()
				...
				if view.response:
					...
					await self._update_ping_cache(db, interaction.guild.id)
					await interaction.followup.send(
						"Pings have been successfully purged."
					)
				elif not view.response:
					# Action cancelled
					await interaction.followup.send("Purge action cancelled.")
				else:
					# Notify the user that the action timed out
					await interaction.followup.send(
						"Pending ping purge has timed out", ephemeral=True
					)

			else:
				# Did not find any pings matching search criteria
				await interaction.response.send_message(
					f"{interaction.user.mention}, there were no pings found with fewer than `{user_threshold}` users that were last used more than `{days_since_last_use}` days ago."
				)
