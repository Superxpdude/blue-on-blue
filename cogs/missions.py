...
class Missions(slash_util.Cog, name = "Missions"):
	...
	async def audit_backup(self, ctx: commands.Context, * , message: str = ""):
		"""Backup method for missions that time-out on the slash command."""
		modpreset = None
		missionfile = None

		for attach in ctx.message.attachments:
			_fileExt = attach.filename.split(".")[-1].casefold()
			if _fileExt == "pbo":
				missionfile = attach
			elif _fileExt == "html":
				modpreset = attach

		if missionfile is None:
			await ctx.send(f"{ctx.author.mention} You need to attach your mission in .pbo format to submit it!")
			return
		...
		if modpreset is not None:
			...
			if modpreset.filename.split(".")[-1].casefold() != "html":
				await ctx.send(f"{ctx.author.mention} Mod preset files must have a `.html` extension!")
				return
			else:
			...
			if missionfile.filename.split("_",1)[0].casefold() == "modnight":
				await ctx.send(f"{ctx.author.mention} Modnight missions must be submitted with a mod preset `.html` file!")
				return
			else:
		...
		if auditChannel is None:
			await ctx.send(f"{ctx.author.mention} I could not locate the audit channel to submit this mission for auditing. Please contact the bot owner.")
			return
		...
		if message is not None:
			auditMessage += f" Notes from the author below \n```{message}```"
